...
void calcLocalBump(float4 veretxModelPos, float4 vertexModelTangent, in float4x3 mW, out LocalBumpOut lbo) {
    float3 vertexWorldPos = mul(veretxModelPos, mW);

    float3 n[3];

    quatToMatrix(vertexModelTangent, n);
...
#if 0
    float3 s = vertexWorldPos * 0.5 + getTime();
    vertexWorldPos += float3( sin(s.y), sin(s.z), sin(s.x) ) * .1;
#endif

    float3 eye = gCameraWorldPos.xyz - vertexWorldPos;
    lbo.lBump.eye = getLocalVector(eye, n);

    for (int k = 0; k < LocalBump::nLights; ++k) {
        float3 lightWorldDir = sLights[k].worldPos.xyz - vertexWorldPos;
        lbo.lBump.lightsDir[k] = float4(getLocalVector(lightWorldDir, n), 1);
    }

    lbo.pos = mul(float4(vertexWorldPos, 1), mViewProj);

    // r.Debug = r.Pos;

    lbo.lBump.debug = n[0];

    lbo.oFog  = 1; // getFogExp2(pos);
}
